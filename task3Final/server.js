...
app.post("/subscriber", (req, res) => {
  ...
  userID++;
  topics[req.body.topic].push(userID - 1);
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (message) => {
    ...
    if (body.conf === 1) {
      ...
      if (body.auth.type === 1) {
        if (topics[body.auth.topic] === undefined) {
          console.log("invalid topic");
        } else {
          topics[body.auth.topic].forEach((subscriber) => {
            subscribers[subscriber].session.send(body.message);
          });
        }
      }
    } else {
